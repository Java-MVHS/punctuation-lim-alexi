...
public class Punctuation
{
	...
	{
		Punctuation pc = new Punctuation();		//new instance of class Punctuation class pc
		System.out.println("\n\n\n");			//3 new lines
		pc.searchIt();							//call method searchIt
		System.out.println("\n\n\n");			//3 new lines
	}

	public void searchIt()
	{
		String text = new String("Blood, Sweat, and Tears by Winston Churchill " +			//concatonate text String
		...
		getPunctuationWords(text);																	//pass in String text to getPunctuationWords method
		
		
	}
	
	public void getPunctuationWords(String text)							//getPunctuationWords method
	{
		int startIndex = 0;													//intialize startIndex (checking start of text)
		for (int endIndex = 0; endIndex < text.length(); endIndex++)		//for loop, intialize endIndex, end if endIndex < length of text, increment through text
		{
			if (text.charAt(endIndex) == ' ')								//once index gets to word, indicate as word, send to if loop -> checkForPunctuation
			{
				String currentWord = text.substring(startIndex, endIndex);	//set currentWord to substring for startIndex, endIndex, indicating word
				if (checkForPunctuation(currentWord))						//call checkForPunctuation and send in currentWord
				{
					printWords(currentWord);								//call printWords if checkForPunctuation returns true, printing word with punctuation in it
				}
				
				startIndex = endIndex;										//set startIndex to endIndex if current index does not have punctuation
			}
			
		}
	}
	
	public boolean checkForPunctuation(String word)							//checkForPunctuation method
	{
		for (int index = 0; index < word.length(); index++)					//for loop for checking individual word
		{
			char c = word.charAt(index);									//check each index using charAt
			if (c == ',' || c == '.' || c == '/' || c == ':' || c == '\"')	//if punctuation, return true to previous method -> print
			{
				return true;												//return true
			}
			
		}
		return false;														//else, return false -> no punctuation
		
	}
	
	public void printWords(String word)										//print words, take in string
	{
